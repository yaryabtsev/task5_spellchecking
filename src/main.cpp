...
int main(int argc, char* argv[])
{
    ...
    //char wordlist[] = "../../res/wordlist.txt";
    //char input[] = "../../res/test.txt";
    ifstream inf(argv[2]);
    if (!inf)
    {
...
void checkSpelling(ifstream& in, Dictionary& dict)
{
    ...
    while (in)
    {
        ...
        while (ss >> word)
        {
            ...
            cout << "Line " << line_number << ". \"" << _word << "\" not found, perhaps you had in mind: ";
...
string stripPunct(string& s) {
    ...
    while (ispunct(s[s.length() - 1])) // hm... what about a combination of punctuation marks?
        s = s.substr(0, s.length() - 1);
    while (ispunct(s[0])) // "why not?"
